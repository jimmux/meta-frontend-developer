...
describe("Utilities", () => {
  it("can parse local date and time in expected formats", () => {
    ...
    expect(getChicagoDateAndTime(gmtTime))
      .toEqual(["2025-01-01", "06:00"]);
  });
});
...
describe("Form validation functions", () => {
  describe("date", () => {
    ...
    it("disallows past dates", () => {
      ...
      date.setDate(date.getDate() - 1);
      const [tomorrowString] = getChicagoDateAndTime(date);
      expect(dateValidation(tomorrowString)).toEqual("Please select a date of today or later.");
    });
  });
  ...
  describe("date and time", () => {
    ...
    it("disallows past times", () => {
      ...
      dateTime.setMinutes(dateTime.getMinutes() - 1);
      const [todayString, nowString] = getChicagoDateAndTime(dateTime);
      expect(dateAndTimeValidation(todayString, nowString)).toEqual("Please select a future date and time.");
    });
  });
  ...
  describe("name", () => {
    ...
    it("disallows names with only 1 characters", () => {
      expect(nameValidation("J")).toEqual("Name must be at least two characters.");
    });

    it("ignores leading and trailing whitespace", () => {
      expect(nameValidation(" X ")).toEqual("Name must be at least two characters.");
    });

    it("disallows empty names", () => {
      expect(nameValidation("")).toEqual("Please enter a guest name for this booking.");
    });
  ...
  describe("number", () => {
    ...
    it("disallows bookings for no people", () => {
      expect(numberValidation(0)).toEqual("Bookings require at least one guest.");
    });

    it("disallows bookings for more than 10 guests", () => {
      expect(numberValidation(11)).toEqual("Bookings are for at most ten guests.");
    });
  });
});