import { initialiseTimes, updateTimes } from "./index";

describe("initialiseTimes", () => {
  it("uses times for today", () => {
    const todayString = new Date().toJSON().split("T").shift();
    const expectedTimes = updateTimes(
      {},
      { type: "changeDate", value: todayString }
    );
...
describe("updateTimes", () => {
  ...
  it("handles unknown actions", () => {
    expect(() => updateTimes([], { type: null, value: "2025-05-27" })).toThrow(
      "Unknown action."
    );
  });
});
